...
var spanNumero = '<span class="box">##numero##<span class="close" onclick="eleminarCaja(##posicion##, event)">x</span></span>'
...
function shortBubble() {
    ...
    //pasar al array ordenado
    arrOrd = [];
    let i = 0;
    let posicionOrd = 0;
    do {
        if (arrDes[i] != undefined) {
            arrOrd[posicionOrd] = arrDes[i];
            posicionOrd++;
        }
        i++;
    } while (i < arrDes.length);
    ...
    if (arrOrd.length > 1) {
        ...
        do {
            ...
            for (i = 0; i < arrOrd.length - 1; i++) {

                if (parseFloat(arrOrd[i]) > parseFloat(arrOrd[i + 1])) {
                    var temp;
                    temp = arrOrd[i];
                    arrOrd[i] = arrOrd[i + 1];
                    arrOrd[i + 1] = temp;
                    ...
                    console.info("Se ha cambiado el elemento " + i + " por el elemento " + (i + 1) + " // Array " + arrOrd);
                }
...
function sacarNumeros() {
    divNumOrd.innerHTML = '';
    for (i = 0; i < arrOrd.length; i++) {
        divNumOrd.innerHTML += spanNumero.replace('##numero##', arrOrd[i]);
    }
}
 ...
 */
function eleminarCaja(posicion, e) {

    //TODO

    delete arrDes[posicion];
    console.log(arrDes);
    console.log(e);
