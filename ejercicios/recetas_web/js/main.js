...
var rIngredientes = $('#ingredientes');
...
function nuevaReceta() {
    ...
    if (validarCV()) {
        ...
        console.log(rIngredientes.val());

        receta.ingredientes = [];
        $.each(rIngredientes.val().split(/\n/), function (i, line) {
            if (line) {
                receta.ingredientes.push(line);
            }
        });
        console.log(receta.ingredientes);

        nuevoThumbnail(receta, recetas.length);
        recetas.push(receta);
...
function nuevoThumbnail(receta, index) {
    ...
    var rThumbnail = `
                <div class="thumbnail box" data-id="${index}">
                    <img class="img-responsive" src="${receta.foto}" alt="${receta.nombre}">
                    <div class="caption">
                        <h3>${receta.nombre}</h3>
                        <div class="info">
                            <p class="likes"><i class="fa fa-heart" aria-hidden="true"></i> ${receta.likes}</p>
                            <p class="cocinero">${receta.cocinero}</p>
                        </div>
                    </div>
                    <div class="ingredientes">
                        <span role="button" data-toggle="modal" data-target="#ingredientesModal" onclick="crearModalJquery(${index})"><i class="fa fa-eye" aria-hidden="true"></i> Ingredientes</span>
                    </div>
                    <span class="close"><i class="fa fa-times-circle" aria-hidden="true" onclick="eleminarCaja(${index})"></i></span>
                </div>
                `;
...
/*rellena modal con jquery*/
function crearModalJquery(index) {

    console.log(recetas[index]);

    var recetaActual = recetas[index];

    var contenido = `<div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="myModalLabel">${recetaActual.nombre}</h4>
                    </div>
...
function resetearBordes() {
    rNombre.style.border = '1px solid #CCCCCC';
    rFoto.style.border = '1px solid #CCCCCC';
    rLikes.style.border = '1px solid #CCCCCC';
    rCocinero.style.border = '1px solid #CCCCCC';
}

/* funcion que valida el form con check validity*/
function validarCV() {
    var form = document.getElementById('formulario');
    return (form.checkValidity());
}

/*function crearFormModal() {
